...
const code = `import { useState, useEffect, useContext, useMemo } from 'react';
                ...
                <div key={deal._id} onClick={() => navigate(\`/deals/\${deal._id}\`)} className="flex items-center gap-4 p-4 hover:bg-slate-50 rounded-2xl border border-slate-100 transition-colors cursor-pointer group">
                  ...
                  <div className="text-right shrink-0 flex flex-col items-end gap-2">
                    ...
                    <span className={\`text-xs font-bold px-3 py-1 rounded-full \${
                      deal.status === 'Completed' || deal.status === 'Accepted' ? 'bg-[#DCFCE7] text-[#16A34A]' : 
...
fs.writeFileSync('src/pages/FarmerDashboard.jsx', code);
